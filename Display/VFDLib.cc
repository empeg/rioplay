...
/* SETCLIPAREA
   ...
   Notes:
    ...
    display area. */
void VFDLib::setClipArea(int xLeft, int yTop, int xRight, int yBottom) {
    /* sanity checks */
...
void VFDLib::clear(char shade) {
    drawSolidRectangleUnclipped(g_clipXLeft, g_clipYTop,
...
/* DRAWPOINTCLIPPED
   ...
   Notes:
    ...
    not be affected. */
void VFDLib::drawPointClipped(int xPos, int yPos, char shade) {
    if (xPos >= g_clipXLeft && xPos < g_clipXRight &&
            yPos >= g_clipYTop && yPos < g_clipYBottom) {

        drawPointUnclipped(xPos, yPos, shade);
    }
}
...
void VFDLib::drawPointUnclipped(int xPos, int yPos, char shade) {
    int byteOffset = (yPos << 6) + (xPos >> 1);
    if (xPos & 0x1) { /* RHS pixel */
        DisplayMemMap[byteOffset] &= MASK_LO_NYBBLE;
        DisplayMemMap[byteOffset] |= (shade << 4);
    }
    else { /* LHS pixel */
        DisplayMemMap[byteOffset] &= MASK_HI_NYBBLE;
        DisplayMemMap[byteOffset] |= shade;
    }
}
...
/* DRAWLINEHORIZCLIPPED
   ...
   Notes:
    This method is clipped so that any pixels falling outside the clip area will
    not be affected. */
void VFDLib::drawLineHorizClipped(int yPos, int xLeft, int xRight,
                                  char shade) {
    if (xLeft < xRight &&
        ...
        drawLineHorizUnclipped(yPos, left,
                               (xRight < g_clipXRight ?
...
/* DRAWLINEHORIZUNCLIPPED
   ...
   Notes:
    This method is unclipped for speed, but if any pixels fall outside the
    display there will be adverse effects. */
void VFDLib::drawLineHorizUnclipped(int yPos, int xLeft,
                                    int length, char shade) {
    char doubleShade = shade | (shade << 4);
    char *buffer = DisplayMemMap;
...
/* DRAWLINEVERTCLIPPED
   ...
   Notes:
    This method is clipped so that any pixels falling outside the clip area will
    not be affected. */
void VFDLib::drawLineVertClipped(int xPos, int yTop, int yBottom,
                                 char shade) {
    if (yTop < yBottom &&
...
/* DRAWLINEVERTUNCLIPPED
   ...
   Notes:
    This method is unclipped for speed, but if any pixels fall outside the
    display there will be adverse effects. */
void VFDLib::drawLineVertUnclipped(int xPos, int yTop, int length,
                                   char shade) {
    char *buffer = DisplayMemMap;
...
/* DRAWLINECLIPPED
   ...
   Notes:
    This method is clipped so that any pixels falling outside the clip area will
    not be affected. */
void VFDLib::drawLineClipped(int x0, int y0, int x1, int y1,
                             char shade) {
    int dx = x1 - x0;
    int dy = y1 - y0;
    ...
    if (dx > dyAbs) { /* hmax line */
        ...
        drawLineHMaxClipped(x0 + xDelta, y0, dx, dy,
                            ((2 + (xDelta << 1)) * dyAbs) -
                            ((1 + (yDelta << 1)) * dx),
                            xMax, yMax, shade);
    }
        ...
        if (distToVEntry > 0) {
            ...
            xDelta += temp / dyAbs;
            if (temp % dyAbs > (dyAbs >> 1))
                xDelta++; /* round up */
            yDelta += distToVEntry;
            if (distToHExit - xDelta < 0)
                return ;
        }
        ...
        drawLineVMaxClipped(x0 + xDelta, y0, dx, dy,
                            ((2 + (yDelta << 1)) * dx) -
                            ((1 + (xDelta << 1)) * dyAbs),
                            xMax, yMax, shade);
    }
...
/* DRAWLINEUNCLIPPED
   ...
   Notes:
    This method is unclipped for speed, but if any pixels fall outside the
    display there will be adverse effects. */
void VFDLib::drawLineUnclipped(int x0, int y0, int x1, int y1,
                               char shade) {
    int dx = x1 - x0;
    int dy = y1 - y0;
    ...
    if (dx == 0) {
        /* vertical line */
        if (dy >= 0)
            drawLineVertUnclipped(x0, y0, dy + 1, shade);
        else
            drawLineVertUnclipped(x0, y1, -dy + 1, shade);
        return ;
    }
    if (dy == 0) {
        /* horizontal line */
        if (dx >= 0)
            drawLineHorizUnclipped(y0, x0, dx + 1, shade);
        else
            drawLineHorizUnclipped(y0, x1, -dx + 1, shade);
        return ;
    }
    if (dx < 0) {
        /* swap points */
    ...
    if (dy < 0) {
        if (dx > -dy)
            drawLineHMaxUnclipped(x0, y0, dx, dy, shade);
        else
            drawLineVMaxUnclipped(x0, y0, dx, dy, shade);
    }
    else {
        if (dx > dy)
            drawLineHMaxUnclipped(x0, y0, dx, dy, shade);
...
/* DRAWOUTLINERECTANGLECLIPPED
   ...
   Notes:
    This method is clipped so that any pixels falling outside the clip area will
    not be affected. */
void VFDLib::drawOutlineRectangleClipped(int xLeft, int yTop,
        int xRight, int yBottom, char shade) {
    drawLineHorizClipped(yTop, xLeft, xRight, shade); /* top */
    drawLineVertClipped(xLeft, yTop, yBottom, shade); /* left */
    drawLineHorizClipped(yBottom - 1, xLeft, xRight,
                         shade); /* bottom */
    drawLineVertClipped(xRight - 1, yTop, yBottom,
                        shade); /* right */
}
...
/* DRAWOUTLINERECTANGLEUNCLIPPED
   ...
   Notes:
    This method is unclipped for speed, but if any pixels fall outside the
    display there will be adverse effects. */
void VFDLib::drawOutlineRectangleUnclipped(int xLeft, int yTop,
        int xWidth, int yHeight, char shade) {
    drawLineHorizUnclipped(yTop, xLeft, xWidth, shade); /* top */
    drawLineVertUnclipped(xLeft, yTop, yHeight, shade); /* left */
    drawLineHorizUnclipped(yTop + yHeight - 1, xLeft, xWidth,
                           shade); /* bottom */
    drawLineVertUnclipped(xLeft + xWidth - 1, yTop, yHeight,
                          shade); /* right */
}
...
/* DRAWSOLIDRECTANGLECLIPPED
   ...
   Notes:
    This method is clipped so that any pixels falling outside the clip area will
    not be affected. */
void VFDLib::drawSolidRectangleClipped(int xLeft, int yTop,
                                       int xRight, int yBottom, char shade) {
    int width, height;
    if (xLeft < g_clipXLeft)
        xLeft = g_clipXLeft;
    if (yTop < g_clipYTop)
        yTop = g_clipYTop;
    if (xRight > g_clipXRight)
        xRight = g_clipXRight;
    if (yBottom > g_clipYBottom)
        yBottom = g_clipYBottom;
    width = xRight - xLeft;
    height = yBottom - yTop;
    if (width < 1 || height < 1)
        return ;
    drawSolidRectangleUnclipped(xLeft, yTop, width, height,
                                shade);
}
...
/* DRAWSOLIDRECTANGLEUNCLIPPED
   ...
   Notes:
    This method is unclipped for speed, but if any pixels fall outside the
    display there will be adverse effects. */
void VFDLib::drawSolidRectangleUnclipped(int xLeft, int yTop,
        int xWidth, int yHeight, char shade) {
    char *buffer = DisplayMemMap;
    int widthRemaining;
    char doubleShade = shade | (shade << 4);
...
/* INVERTRECTANGLECLIPPED
   ...
   Notes:
    This method is clipped so that any pixels falling outside the clip area will
    not be affected. */
void VFDLib::invertRectangleClipped(int xLeft, int yTop,
    ...
    if (width < 1 || height < 1)
        return ;
    invertRectangleUnclipped(xLeft, yTop, width, height);
}
...
/* INVERTRECTANGLEUNCLIPPED
   ...
   Notes:
    This method is unclipped for speed, but if any pixels fall outside the
    display there will be adverse effects. */
void VFDLib::invertRectangleUnclipped(int xLeft, int yTop,
...
void VFDLib::drawOutlineEllipseClipped(int cX, int cY,
    ...
    if ((cX - a) > g_clipXLeft && (cX + a) < g_clipXRight &&
        ...
        drawOutlineEllipseUnclipped(cX, cY, a, b, shade);
        return ;
    }
    ...
    drawPixel4WaySymmetricClipped(cX, cY, x, y, shade);
...
/* DRAWOUTLINEELLIPSEUNCLIPPED
   ...
   Notes:
    ...
    display there will be adverse effects.
    Uses an incremental integer midpoint algorithm for speed. */
void VFDLib::drawOutlineEllipseUnclipped(int cX, int cY,
        int a, int b, char shade) {
    int d2, deltaS;
    ...
    drawPixel4WaySymmetricUnclipped(cX, cY, x, y, shade);
...
/* DRAWSOLIDELLIPSECLIPPED
   ...
   Notes:
    ...
    not be affected.
    Uses an incremental integer midpoint algorithm for speed. */
void VFDLib::drawSolidEllipseClipped(int cX, int cY,
                                     int a, int b, char shade) {
    ...
    if ((cX - a) > g_clipXLeft && (cX + a) < g_clipXRight &&
        ...
        drawSolidEllipseUnclipped(cX, cY, a, b, shade);
        return ;
    }
    ...
    while (a2_2 * y - a_2 > b2_2 * (x + 1)) {
        if (d1 < 0) { /* choose E */
            ...
            deltaE += b8_2;
            deltaSE += a8_2plusb8_2;
            drawLineHorizClipped(cY + y, cX - x, cX + x, shade);
            drawLineHorizClipped(cY - y, cX - x, cX + x, shade);
            y--;
        }
        x++;
    ...
    drawLineHorizClipped(cY, cX - x, cX + x, shade);
}
...
/* DRAWSOLIDELLIPSEUNCLIPPED
   ...
   Notes:
    ...
    display there will be adverse effects.
    Uses an incremental integer midpoint algorithm for speed. */
void VFDLib::drawSolidEllipseUnclipped(int cX, int cY,
                                       int a, int b, char shade) {
    ...
    while (a2_2 * y - a_2 > b2_2 * (x + 1)) {
        if (d1 < 0) { /* choose E */
            ...
            deltaE += b8_2;
            deltaSE += a8_2plusb8_2;
            drawLineHorizUnclipped(cY + y, cX - x, x << 1, shade);
            drawLineHorizUnclipped(cY - y, cX - x, x << 1, shade);
            y--;
        }
        x++;
    ...
    drawLineHorizUnclipped(cY, cX - x, x << 1, shade);
}
...
/* DRAWOUTLINEPOLYGONCLIPPED
   ...
   Notes:
    ...
    the last point. */
void VFDLib::drawOutlinePolygonClipped(int *coordsData,
    ...
    for (currIndex = 0; currIndex <= lastIndex;) {
        ...
        drawLineClipped(x0, y0, x1, y1, shade);
    }
}
...
/* DRAWOUTLINEPOLYGONUNCLIPPED
   ...
   Notes:
    ...
    Polygons are closed automatically - the first point is drawn connected to
    the last point. */
void VFDLib::drawOutlinePolygonUnclipped(int *coordsData,
        int numPoints, char shade) {
    ...
    for (currIndex = 0; currIndex <= lastIndex;) {
        ...
        prevX = x1 = coordsData[currIndex++];
        prevY = y1 = coordsData[currIndex++];
        drawLineUnclipped(x0, y0, x1, y1, shade);
    }
}
...
/* DRAWSOLIDPOLYGON
   ...
   Notes:
    ...
    Polygons are closed automatically - the first point is drawn connected to
    the last point. */
void VFDLib::drawSolidPolygon(int *coordsData, int numPoints,
                              char shade) {
    ...
    for (currIndex = 0; currIndex <= lastIndex;) {
        ...
        if (y0 == y1)
            continue; /* skip horizontal spans */
        ...
        if (y1 < g_clipYTop || y0 >= g_clipYBottom)
            continue;
    ...
    AEtable = NULL;
    if (polyMaxY > g_clipYBottom - 1)
        polyMaxY = g_clipYBottom - 1;
...
int VFDLib::registerFont(char *bfFileName, int fontSlot) {
    ...
    fBitmap = (char *) malloc(fBitmapSize);
    for (i = 0; i < fBitmapSize; i++)
        fBitmap[i] = 0; /* clear memory */
...
/*                if(((unsigned char) scanline & 0x03) == 0x03) {
                    *(fBitmap + 5 + scanLineSkip + ((cInfo[i].offset + x) >> 4)) |=
                           ((cInfo[i].offset + x) & 0x0f);
                }*/
...
/* UNREGISTERFONT
   ...
   Notes:
    ...
    been registered. */
void VFDLib::unregisterFont(int fontSlot) {
...
void VFDLib::unregisterAllFonts() {

    int i;
    for (i = 0; i < NUM_FONT_SLOTS; i++)
        unregisterFont(i);
}
...
void VFDLib::drawText(char *string, int xLeft, int yTop,
    ...
    while (*string != '\0' && xLeft < g_clipXRight) {
        ...
        if (charIndex >= 0 && charIndex < g_fontRegistry[fontSlot].numOfChars) {
            ...
            if (ci.width > 0) {
                drawBitmap((unsigned char *) g_fontRegistry[fontSlot].fBitmap,
...
/* GETTEXTHEIGHT
   ...
   Parameters:
    fontSlot - the slot number of a font that has been registered */
int VFDLib::getTextHeight(int fontSlot) {
...
/* GETTEXTWIDTH
   ...
   Parameters:
    ...
    fontSlot - the slot number of a font that has been registered */
int VFDLib::getTextWidth(char *string, int fontSlot) {
...
int VFDLib::getStringIndexOfCutoffWidth(char *string, int fontSlot, int width) {
    ...
    while (*string != '\0') {
        ...
        if (charIndex >= 0 && charIndex < g_fontRegistry[fontSlot].numOfChars) {
            ...
            currWidth += ci.width;
            if (currWidth > width)
                return index; /* found cutoff character */
        }
        string++;
        index++;
    }
...
void VFDLib::drawBitmap(unsigned char *bitmap,
                        int destX, int destY,
    ...
    if (width > bitmapWidth)
        width = bitmapWidth;
    if (destX + width > g_clipXRight)
        width = g_clipXRight - destX;

    /* clip bottom */
    if (height > bitmapHeight)
        height = bitmapHeight;
    if (destY + height > g_clipYBottom)
        height = g_clipYBottom - destY;

    if (width > 0 && height > 0) {
        switch (bitmap[0]) {
        case BITMAP_1BPP:
            drawBitmap1BPP(bitmap + 5, bitmapWidth, bitmapHeight, destX, destY,
                           sourceX, sourceY, width, height, shade, isTransparent);
            break;
        case BITMAP_2BPP:
            drawBitmap2BPP(bitmap + 5, bitmapWidth, bitmapHeight, destX, destY,
                           sourceX, sourceY, width, height, shade, isTransparent);
            break;
        case BITMAP_4BPP:
            drawBitmap4BPP(bitmap + 5, bitmapWidth, bitmapHeight, destX, destY,
                           sourceX, sourceY, width, height, shade, isTransparent);
            break;
        }
    }
...
void VFDLib::drawPixel4WaySymmetricClipped(int cX, int cY,
        int x, int y, char shade) {

    drawPointClipped(cX + x, cY + y, shade);
    drawPointClipped(cX + x, cY - y, shade);
    drawPointClipped(cX - x, cY + y, shade);
    drawPointClipped(cX - x, cY - y, shade);
}

void VFDLib::drawPixel4WaySymmetricUnclipped(int cX, int cY,
        int x, int y, char shade) {

    drawPointUnclipped(cX + x, cY + y, shade);
    drawPointUnclipped(cX + x, cY - y, shade);
    drawPointUnclipped(cX - x, cY + y, shade);
    drawPointUnclipped(cX - x, cY - y, shade);
}

void VFDLib::drawLineHMaxClipped(int x0, int y0, int dx, int dy,
...
void VFDLib::drawLineVMaxClipped(int x0, int y0, int dx, int dy,
                                 int d, int xMax, int yMax, char shade) {
    char *buffer = DisplayMemMap;
...
void VFDLib::drawLineHMaxUnclipped(int x0, int y0,
...
void VFDLib::drawLineVMaxUnclipped(int x0, int y0,
                                   int dx, int dy, char shade) {
    char *buffer = DisplayMemMap;
...
void VFDLib::drawBitmap1BPP(unsigned char *bitmap,
                            int bitmapWidth, int bitmapHeight,
                            int destX, int destY,
                            int sourceX, int sourceY,
                            int width, int height,
                            signed char shade, int isTransparent) {
    unsigned char *buffer = (unsigned char *) DisplayMemMap;
    int startPos, widthLeft;
    unsigned char *startBitmap, *startBuffer;
...
void VFDLib::drawBitmap2BPP(unsigned char *bitmap,
                            int bitmapWidth, int bitmapHeight,
                            int destX, int destY,
                            int sourceX, int sourceY,
                            int width, int height,
    ...
    unsigned char currentBitmap, currentShade;
    unsigned char *startBitmap, *startBuffer;
...
void VFDLib::drawBitmap4BPP(unsigned char *bitmap,
                            int bitmapWidth, int bitmapHeight,
                            int destX, int destY,
                            int sourceX, int sourceY,
                            int width, int height,
    ...
    unsigned char currentShade, currentTrans;
    unsigned char *startBitmap, *startBuffer;
